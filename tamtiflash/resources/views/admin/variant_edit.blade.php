...
<div class="container-fluid pt-4 px-4">
    <div class="col-12">
        <div class="bg-light rounded h-100 p-4">
            <h6 class="mb-4">Sửa biến thể</h6>
            <a class="btn btn-secondary rounded-pill m-2" href="{{ route('admin.product_variant', ['id' => $product->id]) }}">Trở lại</a>
            <form action="{{ route('admin.editVariant', ['id' => $productVariant->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
            ...
            <form action="{{ route('admin.deleteVariant', ['id' => $productVariant->id]) }}" method="post">
                @csrf
                @method('DELETE')

                        ...
                        @else
                            ...
                            @foreach ($sortedStaffs as $staff)
                                @php
                                    ...
                                    $work = $staff->work == '0' ? 'Đang rảnh' : 'Có đơn';
                                    $workColor = $staff->work == '0' ? 'color: green;' : 'color: red;';
                                @endphp

                                <tr data-staff-id="{{ $staff->id }}">
                                    <td style="{{ $style }}">{{ $staff->name }}</td>
                                    <td style="{{ $style }}">{{ $role }}</td>
                                    <td class="status-cell" style="{{ $statusColor }}" data-initial-status="{{ $staff->status }}">{{ $status }}</td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('admin.staff.delete', ['id' => $staff->id]) }}" class="btn btn-sm btn-danger">Xóa</a>
                                    </td>
                                </tr>
                            @endforeach
                        @endif
            ...
            document.querySelectorAll('tr[data-staff-id]').forEach(row => {
                ...
                fetch("{{ route('admin.staff.status', '') }}/" + staffId)
                    ...
                    .then(data => {
                        ...
                        const workText = data.work === 0 ? 'Đang rảnh' : 'Có đơn';
                        const workColor = data.work === 0 ? 'color: green;' : 'color: red;';
                        workCell.innerText = workText;
                        workCell.style = workColor;
                    })
                    .catch(error => {
